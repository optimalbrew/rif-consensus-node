...
* Not simplistic:
  ...
  - Ethereum is complex and RSK Private must handle this complexity and operate correctly and securely 
  - RSK Private code should align with well-established Ethereum abstractions and terminology used in Ethereum specifications 
  - Aim to make the code as simple as possible but no simpler
  
## 2.2 Idiomatic Java 

RSK Private embraces typical Java idioms including using an Object Oriented approach to design. This includes:   
...
>**Note** If you're not sure what idiomatic Java looks like, start by following the typical patterns and naming used in the code.

## 2.3 You Ain't Gonna Need It (YAGNI)

The RSK Private design prioritizes meeting current requirements in the simplest, clearest way over attempting to anticipate future functionality. As a result, RSK Private’s design:
...
* Uses dependency injection
  ...
  - RSK Private does not use a dependency injection framework
...
* Consider exception and error handling as part of the overall design.
...
RSK Private follows the [Google code style](https://google.github.io/styleguide/javaguide.html) and uses spotless to ensure consistency of formatting.
...
Logging is important for understanding what the code is doing at any given time (for example, progress while synchronizing) and investigating defects.  During development, add logging to aid in these cases. 
...
* _Warn_ 
  ...
  Anything that can potentially cause application oddities but from which the node automatically recovers

* _Error_ 

  Any error which is fatal to the operation, but not the nodee itself (for example, missing data)

* _Fatal_  

  An error that forces a shutdown of the node
