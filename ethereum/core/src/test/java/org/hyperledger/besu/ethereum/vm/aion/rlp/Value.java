...
public class Value {
    ...
    public static org.aion.rlp.Value fromRlpEncoded(byte[] data) {

        if (data != null && data.length != 0) {
            org.aion.rlp.Value v = new org.aion.rlp.Value();
            v.init(data);
            return v;
        }
        return null;
    ...
    public Value(Object obj) {
        ...
        if (obj instanceof org.aion.rlp.Value) {
            this.value = ((org.aion.rlp.Value) obj).asObj();
        } else {
            this.value = obj;
        }
    }
    ...
    public org.aion.rlp.Value get(int index) {
        if (isList()) {
            // Guard for OutOfBounds
            List<Object> list = asList();
            if (list.size() <= index) {
                return new org.aion.rlp.Value(null);
            }
            if (index < 0) {
                throw new RuntimeException("Negative index not allowed");
            }
            return new org.aion.rlp.Value(list.get(index));
        }
    ...
    public boolean cmp(org.aion.rlp.Value o) {
        if (o == null) {
            return false;
        }
    ...
    public String toString() {
        ...
        if (isList()) {
            ...
            if (list.length == 2) {
                ...
                org.aion.rlp.Value key = new org.aion.rlp.Value(list[0]);

                byte[] keyNibbles = org.aion.rlp.CompactEncoder.binToNibblesNoTerminator(key.asBytes());
                String keyString = ByteUtil.nibblesToPrettyString(keyNibbles);
                stringBuilder.append(keyString);

                stringBuilder.append(",");

                org.aion.rlp.Value val = new org.aion.rlp.Value(list[1]);
            ...
            for (int i = 0; i < list.length; ++i) {
                org.aion.rlp.Value val = new org.aion.rlp.Value(list[i]);
