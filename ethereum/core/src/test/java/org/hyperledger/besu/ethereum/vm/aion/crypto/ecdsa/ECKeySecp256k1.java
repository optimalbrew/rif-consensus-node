package org.hyperledger.besu.ethereum.vm.aion.crypto.ecdsa;

import org.hyperledger.besu.ethereum.vm.aion.crypto.ECKey;
import org.hyperledger.besu.ethereum.vm.aion.crypto.ISignature;
import org.hyperledger.besu.ethereum.vm.aion.crypto.jce.*;
...
import static org.hyperledger.besu.ethereum.vm.aion.util.bytes.ByteUtil.bigIntegerToBytes;
 ...
 * than requiring the user to provide the expected identity. This code is borrowed from the bitcoinj
 * project and altered to fit Ethereum.<br>
 * See <a href=
...
public class ECKeySecp256k1 implements ECKey, Serializable {
     ...
     * <p>BouncyCastle will be used as the Java Security Provider
     */
    public ECKeySecp256k1(BigInteger priv, ECPoint pub) {
    ...
    public ECKeySecp256k1 fromPrivate(BigInteger privKey) {
        return new ECKeySecp256k1(privKey, CURVE.getG().multiply(privKey));
    }
    ...
    public ECKeySecp256k1 fromPrivateAndPrecalculatedPublic(BigInteger priv, ECPoint pub) {
        return new ECKeySecp256k1(priv, pub);
    }
    ...
    public ECKeySecp256k1 fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        ...
        return new ECKeySecp256k1(new BigInteger(1, priv), CURVE.getCurve().decodePoint(pub));
    }
     ...
     *
     * <p>NOTE: This address computation is Ethereum-specific.
     *
    ...
    public byte[] signatureToAddress(byte[] messageHash, ISignature sig) throws SignatureException {
        return computeAddress(signatureToKeyBytes(messageHash, (ECDSASignature) sig));
    }
     ...
     * @return 20-byte address
     */
    public byte[] recoverAddressFromSignature(int recId, ECDSASignature sig, byte[] messageHash) {
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || !(o instanceof ECKeySecp256k1)) {
            return false;
        }

        ECKeySecp256k1 ecKey = (ECKeySecp256k1) o;
