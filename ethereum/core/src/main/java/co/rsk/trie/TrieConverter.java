...
import co.rsk.db.TrieKeyMapper;
...
public class TrieConverter {
    ...
    private final Map<Keccak256, BytesValue> cacheHashes;
    ...
    public BytesValue getOrchidAccountTrieRoot(Trie src) {
        //Metric metric = profiler.start(Profiler.PROFILING_TYPE.TRIE_CONVERTER_GET_ACCOUNT_ROOT);
        BytesValue trieRoot =  cacheHashes.computeIfAbsent(src.getHash(), k -> {
            Trie trie = getOrchidAccountTrieRoot(src.getSharedPath(), src, true);
            return trie == null ? new Keccak256(Hash.EMPTY_TRIE_HASH).getBytes() : trie.getHashOrchid(true).getBytes();
        });
    ...
    private Trie getOrchidAccountTrieRoot(TrieKeySlice key, Trie src, boolean removeFirst8bits) {
        ...
        //TODO VALIDATE REMASC WONT EXIST HERE
        boolean isRemascAccount = false;

        if ((key.length() == (1 + TrieKeyMapper.SECURE_KEY_SIZE + Address.SIZE) * Byte.SIZE || isRemascAccount) && src.getValue() != null) {
            // We've reached the Account level. From now on everything will be different.
            //TODO: SEE WETHER ACCOUNT IS REALLY NEEDED OR NOT
            RLPInput accountInfo = RLP.input(src.getValue());
            //AccountState astate = new AccountState(src.getValue());
            accountInfo.enterList();
            BigInteger nonce = accountInfo.readBigIntegerScalar();
            Coin balance = RLPUtils.parseCoin(accountInfo.readBigIntegerScalar());

            OrchidAccountState oldState = new OrchidAccountState(nonce, balance); //new OrchidAccountState(astate.getNonce(), astate.getBalance());
            // child1 (0x80) will be the code
            if (child1 != null) {
                oldState.setCodeHash(child1.getValueHash());//TODO DO WE NEED TO COPY THE VALUE?
            } else {
                oldState.setCodeHash(new Keccak256(Keccak256Helper.keccak256(new byte[0])));
            }
            ...
            if (child0 != null) {
                ...
                oldState.setStateRoot(root.getHashOrchid(true));
            } else if (isRemascAccount) {
                oldState.setStateRoot(new Keccak256(Keccak256Helper.keccak256(RLP.encodeOne(BytesValue.wrap(new byte[0])))));
            }

            BytesValue avalue = oldState.getEncoded();
            TrieKeySlice orchidKey;
            if (isRemascAccount) {
                orchidKey = null;//extractOrchidAccountKeyPathFromUnitrieKey(key, sharedPath.length(), RemascTransaction.REMASC_ADDRESS.getBytes().length, TrieKeyMapper.REMASC_ACCOUNT_KEY_SIZE);
            } else {
                orchidKey = extractOrchidAccountKeyPathFromUnitrieKey(key, sharedPath.length(), Address.SIZE, TrieKeyMapper.SECURE_ACCOUNT_KEY_SIZE);
            }

            return new Trie(
                    null, orchidKey, avalue, NodeReference.empty(), NodeReference.empty(),
                    new Uint24(avalue.size()), null
            );
        }
    ...
    private Trie getOrchidStateRoot(
        ...
        BytesValue value = unitrieStorageRoot.getValue();
    ...
    private TrieKeySlice extractOrchidAccountKeyPathFromUnitrieKey(TrieKeySlice key, int sharedPathLength, int addressLengthInBytes, int unitrieKeySizeInBytes) {
        ...
        BytesValue encodedKey = key.slice(key.length() - addressLengthInBytes * Byte.SIZE, key.length()).encode();
        BytesValue orchidTrieSecureKey = Keccak256Helper.keccak256(encodedKey);
    ...
    private TrieKeySlice extractOrchidStorageKeyPathFromUnitrieKey(TrieKeySlice key, TrieKeySlice sharedPath) {
        // 42 = DOMAIN_PREFIX(1) + SECURE_KEY_SIZE(10) + RskAddress(20) + STORAGE_PREFIX(1) + SECURE_KEY_SIZE(10)
        int leadingZeroesToAdd = 42 * Byte.SIZE + Bytes32.SIZE * Byte.SIZE - key.length();
        TrieKeySlice unsecuredKey = key.slice(42 * Byte.SIZE, key.length()).leftPad(leadingZeroesToAdd);
        BytesValue orchidTrieSecureKey = Keccak256Helper.keccak256(unsecuredKey.encode());
