...
public class MultiTrieStore implements TrieStore {
     ...
     * epoch, it'll be reached by the {@link #retrieve(BytesValue) retrieve} method breaking the recursion
     *
     ...
     */
    @Override
    public Trie retrieve(BytesValue rootHash) {
        for (TrieStore epochTrieStore : epochs) {
            BytesValue message = epochTrieStore.retrieveValue(rootHash);
            if (message == null) {
                continue;
            }
        ...
        throw new IllegalArgumentException(String.format(
                "The trie with root %s is missing from every epoch", rootHash.toString()
        ));
    }
    ...
    public BytesValue retrieveValue(BytesValue hash) {
        for (TrieStore epochTrieStore : epochs) {
            BytesValue value = epochTrieStore.retrieveValue(hash);
            if (value != null) {
                return value;
            }
        }
     ...
     */
    public void collect(BytesValue oldestTrieHashToKeep) {
