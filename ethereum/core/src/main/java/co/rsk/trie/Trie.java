...
import co.rsk.core.types.ints.VarInt;
...
public class Trie {
    ...
    private Trie(TrieStore store, TrieKeySlice sharedPath, BytesValue value) {
        this(store, sharedPath, value, NodeReference.empty(), NodeReference.empty(), getDataLength(value), null);
    }

    public Trie(TrieStore store, TrieKeySlice sharedPath, BytesValue value, NodeReference left, NodeReference right, Uint24 valueLength, Keccak256 valueHash) {
        this(store, sharedPath, value, left, right, valueLength, valueHash, null);
    }

    // full constructor
    private Trie(TrieStore store, TrieKeySlice sharedPath, BytesValue value, NodeReference left, NodeReference right, Uint24 valueLength, Keccak256 valueHash, VarInt childrenSize) {
        this.value = value;
        this.left = left;
        this.right = right;
        this.store = store;
     ...
     */
    public static Trie fromMessage(BytesValue message, TrieStore store) {
        Trie trie;
        ...
        if (message.get(0) == ARITY) {
            ...
            trie = fromMessageRskip107(ByteBuffer.wrap(message.getArrayUnsafe()), store);
        }
    ...
    private static Trie fromMessageOrchid(BytesValue message, TrieStore store) {
        int current = 0;
        int arity = message.get(current);
        current += Byte.BYTES;
        ...
        if (hasLongVal) {
            ...
            if (remaining > 0) {
                if (message.size() - current  < remaining) {
                    throw new IllegalArgumentException(String.format(
                            "Left message is too short for value expected:%d actual:%d total:%d",
                            remaining, message.size() - current, message.size()));
                }
    ...
    private static Trie fromMessageRskip107(ByteBuffer message, TrieStore store) {
        ...
        return new Trie(store, sharedPath, BytesValue.wrap(value), left, right, lvalue, valueHash, childrenSize);
    }
    ...
    public Keccak256 getHash() {
        ...
        BytesValue message = this.toMessage();
    ...
    public BytesValue get(BytesValue key) {
        ...
        BytesValue result = node.getValue();
        //profiler.stop(metric);
        return result;
    }
     ...
     */
    public BytesValue get(String key) {
        return this.get(BytesValue.wrap(key.getBytes(StandardCharsets.UTF_8)));
    }
     ...
     */
    public Trie put(BytesValue key, BytesValue value) {
     ...
     * value association
     */
    public Trie put(String key, BytesValue value)
    {
        return put(BytesValue.wrap(key.getBytes(StandardCharsets.UTF_8)), value);
    }
     ...
     *
     */
    public Trie delete(BytesValue key) {
        return put(key, null);
    }

    // This is O(1). The node with exact key "key" MUST exists.
    public Trie deleteRecursive(BytesValue key) {
        TrieKeySlice keySlice = TrieKeySlice.fromKey(key);
    ...
    public Trie delete(String key) {
        return delete(BytesValue.wrap(key.getBytes(StandardCharsets.UTF_8)));
    }
    ...
    public byte[] toMessageOrchid(boolean isSecure) {
        ...
        if (lshared > 0) {
            buffer.put(this.sharedPath.encode().getArrayUnsafe());
        }

        leftHashOpt.ifPresent(h -> buffer.put(h.getBytes().getArrayUnsafe()));

        rightHashOpt.ifPresent(h -> buffer.put(h.getBytes().getArrayUnsafe()));

        if (lvalue.compareTo(Uint24.ZERO) > 0) {
            if (hasLongVal) {
                buffer.put(this.getValueHash().getBytes().getArrayUnsafe());
            }
            else {
                buffer.put(this.getValue().getArrayUnsafe());
            }
        }
    ...
    private void collectKeys(Set<BytesValue> set, TrieKeySlice key, int collectKeyLen) {
    ...
    // Special value Integer.MAX_VALUE means collect them all.
    public Set<BytesValue> collectKeys(int byteSize) {
        Set<BytesValue> set = new HashSet<>();
     ...
     */
    @Nullable
    public Trie find(BytesValue key) {
    ...
    private void internalToMessage() {
        ...
        encoded = BytesValue.wrap(buffer.array());
    }
     ...
     *
     */
    private Trie put(TrieKeySlice key, BytesValue value, boolean isRecursiveDelete) {
    ...
    private static Uint24 getDataLength(BytesValue value) {
        if (value == null) {
            return Uint24.ZERO;
        }

        return new Uint24(value.size());
    }

    private Trie internalPut(TrieKeySlice key, BytesValue value, boolean isRecursiveDelete) {
        ...
        if (sharedPath.length() >= key.length()) {
            ...
            // obviously are not.
            if (this.valueLength.equals(getDataLength(value)) && this.getValue().equals(value)) {
        ...
        if (isEmptyTrie()) {
            ...
            return new Trie(this.store, key, value.copy());
    ...
    private void checkValueLength() {
        if (value != null && value.size() != valueLength.intValue()) {
            // Serious DB inconsistency here
            throw new IllegalArgumentException(INVALID_VALUE_LENGTH);
        }
    ...
    private static Keccak256 readHash(BytesValue bytes, int position) {
        int keccakSize = Keccak256Helper.DEFAULT_SIZE_BYTES;
        if (bytes.size() - position < keccakSize) {
            throw new IllegalArgumentException(String.format(
                    "Left message is too short for hash expected:%d actual:%d total:%d",
                    keccakSize, bytes.size() - position, bytes.size()
            ));
        }
    ...
    public String toString() {
        String s = printParam("TRIE: ", "value", getValue().getArrayUnsafe());
        ...
        s = printParam(s, "encodedSharedPath", sharedPath.encode().getArrayUnsafe());
    ...
    public static class IterationElement {
        ...
        public String toString() { //BytesValue::toString() is hex representation
            byte[] encodedFullKey = nodeKey.encode().getArrayUnsafe();
    ...
    private static class SharedPathSerializer {
        ...
        private static TrieKeySlice deserialize(ByteBuffer message, boolean sharedPrefixPresent) {
            ...
            return TrieKeySlice.fromEncoded(BytesValue.wrap(encodedKey), 0, lshared, lencoded);
        }
    }
    ...
    @Nullable
    public List<Trie> getNodes(BytesValue key) {
        return findNodes(key);
    }

    @Nullable
    public List<Trie> getNodes(String key) {
        return this.getNodes(BytesValue.wrap(key.getBytes(StandardCharsets.UTF_8)));
    }

    @Nullable
    private List<Trie> findNodes(BytesValue key) {
