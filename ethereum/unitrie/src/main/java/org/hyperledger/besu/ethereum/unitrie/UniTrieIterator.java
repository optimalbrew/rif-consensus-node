 ...
 *
 * <p>Given a valid path it iterates the Unitrie from it, invoking a callback for each leaf. Such
 * callback receives the full path to a leaf plus the leaf itself. It's up to the callback to
 * determine whether the iteration must continue or stop.
 *
 * <p>In general, this iterator is used to collect a range of leaves for a given Unitrie.
 *
 * @author ppedemon
 */
public class UniTrieIterator implements UniPathVisitor {
   ...
   *
   * <ul>
   *   <li>SEARCHING: look for initial leaf to collect.
   *   <li>CONTINUE: first leaf to collect found, from now collect every leaf we find.
   *   <li>STOP: stop iteration, ignore further leaves.
   * </ul>
   */
  public enum State {
    SEARCHING,
