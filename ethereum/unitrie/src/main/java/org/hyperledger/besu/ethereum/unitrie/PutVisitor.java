...
public class PutVisitor implements UniPathVisitor {
    ...
    public UniNode visit(final BranchUniNode node, final BytesValue path) {
        ...
        // If we get here then commonPath.size() == nodePath.size() and commonPath.size() < path.size().
    ...
    /**
     * Produce a split without a new leaf. This will happen because the common path is equal
     * to the new leaf path, so the split root must hold the new value.
     *
    ...
    /**
     * Produce a split with a new leaf. This will happen because the new leaf path is larger
     * than the common path, so the split root must have as children the updated node and
     * the a new leaf holding the value to insert.
     *
     * @param commonPath      split's common path (will be associated to the split's root)
     * @param value           new value to insert
     * @param updatedNode     original node, now holding the suffix of the common path
