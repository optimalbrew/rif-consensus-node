...
public class UniNodeEncoding {
     ...
     */
    private void encodePath(final int pathSizeInBits, final BytesValue encodedPath, final ByteBuffer buffer) {
        if (pathSizeInBits == 0) {
            return;
        }
     ...
     * @return  decode node
     */
    UniNode decode(final BytesValue value, final DataLoader loader, final StoredUniNodeFactory nodeFactory) {
    ...
    private UniNode decode(
        ...
        UniNode rightChild = decodeChild(buffer, hasRightChild, rightChildEmbedded, loader, nodeFactory);
        ...
        return new BranchUniNode(path, valueWrapper, leftChild, rightChild, childrenSize, loader, nodeFactory);
    }
     ...
     * @param hasChild  whether we should actually decode a child (otherwise return a {@link NullUniNode})
