...
public class UniTrieKeyMapper {
    ...
    public BytesValue getAccountStorageKey(final Address address, final BytesValue subkey) {
        ...
        ByteBuffer buffer = ByteBuffer.allocate(accountStorageKeySize(addressBytes, strippedSubkeyBytes));
    ...
    private int accountStorageKeySize(final byte[] addressBytes, final byte[] strippedSubkeyBytes) {
        return accountStoragePrefixKeySize(addressBytes) + HASH_DIGEST_PREFIX_SIZE + strippedSubkeyBytes.length;
    }
