...
public class UniTrieNodeDecoderTest {
    ...
    public void decodeUniNodes() {
        ...
        trie.put(BytesValue.fromHexString("0x" + Strings.repeat("01", 32)),
        ...
        final BytesValue encodedRoot = storage.get(trie.getRootHash()).orElseGet(() -> {
...

        {
            ...
            assertThat(collectValues(children)).containsExactlyInAnyOrder(BytesValue.of(1), BytesValue.of(2));
        }
        {
            final List<UniNode> children = decoder.decodeNodes(nodes.get(2).getEncoding());
            assertThat(children.size()).isEqualTo(3);
            assertThat(collectValues(children)).containsExactlyInAnyOrder(
                    BytesValue.of(3),
                    BytesValue.fromHexString("0x" + Strings.repeat("bad", 50)));
        }
    }
    ...
    public void breathFirstDecoder_fullTrie() {
        ...
        final List<UniNode> depth0Nodes = UniTrieNodeDecoder
                .breadthFirstDecoder(storage::get, trie.getRootHash(), 0)
        ...
        final List<UniNode> depth0And1Nodes = UniTrieNodeDecoder
                .breadthFirstDecoder(storage::get, trie.getRootHash(), 1)
                .collect(Collectors.toList());
        ...
        List<Bytes32> actualNodeHashes = depth0And1Nodes.subList(1, expectedNodeCount).stream()
                .map(UniNode::getHash).collect(Collectors.toList());
        assertThat(actualNodeHashes).isEqualTo(expectedNodesHashes);

        // Decode full Unitrie
        final List<UniNode> allNodes = UniTrieNodeDecoder
                .breadthFirstDecoder(storage::get, trie.getRootHash())
                .collect(Collectors.toList());
    ...
    public void breadthFirstDecode_partialTrie() {
        ...
        // Get root node
        UniNode rootNode =
                UniTrieNodeDecoder.breadthFirstDecoder(fullStorage::get, trie.getRootHash()).findFirst().get();

        // Decode partially available trie
        partialStorage.put(trie.getRootHash(), rootNode.getEncoding());
        final List<UniNode> allDecodableNodes = UniTrieNodeDecoder
                .breadthFirstDecoder(partialStorage::get, trie.getRootHash())
                .collect(Collectors.toList());
    ...
    public void breadthFirstDecode_emptyTrie() {
        List<UniNode> result =
                UniTrieNodeDecoder
                        .breadthFirstDecoder(__ -> Optional.empty(), UniNodeEncoding.NULL_UNINODE_HASH)
                        .collect(Collectors.toList());
    ...
    public void breadthFirstDecode_singleNodeTrie() {
        ...
        List<UniNode> result = UniTrieNodeDecoder
                .breadthFirstDecoder(storage::get, trie.getRootHash())
                .collect(Collectors.toList());
    ...
    public void breadthFirstDecode_unknownTrie() {
        ...
        List<UniNode> result = UniTrieNodeDecoder
                .breadthFirstDecoder(__ -> Optional.empty(), randomRootHash)
                .collect(Collectors.toList());
        assertThat(result.size()).isEqualTo(0);
    }
