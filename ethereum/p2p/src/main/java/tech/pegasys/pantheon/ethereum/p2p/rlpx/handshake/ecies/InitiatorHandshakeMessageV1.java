...
public final class InitiatorHandshakeMessageV1 implements InitiatorHandshakeMessage {
  public static final int MESSAGE_LENGTH =
      SIGNATURE_LENGTH + HASH_EPH_PUBKEY_LENGTH + PUBKEY_LENGTH + NONCE_LENGTH + TOKEN_FLAG_LENGTH;
  ...
  public static InitiatorHandshakeMessageV1 decode(
    ...
    int offset = 0;
    final SECP256K1.Signature signature =
        SECP256K1.Signature.decode(bytes.slice(offset, SIGNATURE_LENGTH));
    final Bytes32 ephPubKeyHash =
        Bytes32.wrap(bytes.slice(offset += SIGNATURE_LENGTH, HASH_EPH_PUBKEY_LENGTH), 0);
    final SECP256K1.PublicKey pubKey =
        SECP256K1.PublicKey.create(bytes.slice(offset += HASH_EPH_PUBKEY_LENGTH, PUBKEY_LENGTH));
    final Bytes32 nonce = Bytes32.wrap(bytes.slice(offset += PUBKEY_LENGTH, NONCE_LENGTH), 0);
  ...
  public BytesValue encode() {
    ...
    ephPubKeyHash.copyTo(bytes, SIGNATURE_LENGTH);
    pubKey.getEncodedBytes().copyTo(bytes, SIGNATURE_LENGTH + HASH_EPH_PUBKEY_LENGTH);
    nonce.copyTo(bytes, SIGNATURE_LENGTH + HASH_EPH_PUBKEY_LENGTH + PUBKEY_LENGTH);
